#!/usr/bin/env python
from numpy import *
import math,sys,pdb,time,re,os,copy#,random
...
def printusage():
 ...
 sys.exit()
################################functions
def random_array_maker_and_row_sorter(start,stop,step):#,row_num,col_num):
    global simplex
    global rep_num
    global conv_const
    #rand_list=[random.uniform(start,stop) for _ in xrange((row_num)*(col_num))]
    equi_vector=my_range(start,stop,step)
    
    print "equi_vector: ", equi_vector/conv_const
    for i in range(rep_num-2):
        #print(i)
        simplex[:,i]=random.normal(equi_vector[i],step/50.00,rep_num-1)
    ...
    #while simplex_scores_vector[0,0]>precision:
        ...
        if (X_r_score<=simplex_scores_vector[rep_num-2,0] and X_r_score>=simplex_scores_vector[0,0]):
            ...
            if (X_e_score<X_r_score):
                simplex[rep_num-2,:]=X_e
                simplex_scores_vector[rep_num-2]=X_e_score
            else:
                simplex[rep_num-2,:]=X_r
                simplex_scores_vector[rep_num-2]=X_r_score
        else:#contraction
...
random_array_maker_and_row_sorter(win_min,win_max,initial_difference)#,rep_num-1,rep_num-2)
print(type(simplex))
...
N_M_minimizer()
feed_for_sd_method=simplex[0,:]
print "NM_minimization_results: ",score_function(feed_for_sd_method)
result[1:rep_num-1]=BB_gradient_desc(feed_for_sd_method,"minimize")
