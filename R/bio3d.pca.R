name_format="apo_chol_nowat"

dcdfile="/Scr/danial/DAT/apo_cholesterol/general_analysis_1/all_traj_apo_chl1.dcd"
...
plot.bio3d(pc$au[, 1], ylab = "PC1/PC2 (A)", xlab = "Residue Position", typ = "l",col="red")
points(pc$au[, 2], typ = "l", col = "blue")
legend("topleft",c("PCA1","PCA2"),col=c("red","blue"),lty=c(1,1))
dev.off()
...
pdf(file=paste(name_format,".CCA.pdf",sep=""))
...
#################################Radius of Gyration
postscript(file=paste(name_format,".RGY.eps",sep=""))

rg <- rgyr(xyz)

plot(rg, typ = "l", ylab = "Radius of Gyration (A)", xlab = "Frame No.")
points(lowess(rg), typ = "l", col = "red", lty = 2, lwd = 2)
dev.off()
